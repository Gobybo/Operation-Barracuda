...
Class conteneurGenre
	...
	public function listeDesGenres()
		...
		foreach ($this->lesGenres as $unGenre)
			{	
				$liste = $liste.'<div class="col-xs-4 col-sm-4 col-md-4"><a href="Vues/voirSupport.php?idGenre='.$unGenre->getIdGenre().'"><img src="'.$unGenre->getCheminImageGenre().'" class="rounded mx-auto d-block" style="width : 200px; length : 200px;"></a><figcaption class="figure-caption text-center"><a href="index.php?login='.$_GET['login'].'&vue=Videotheque&action=choixGenre&idGenre='.$unGenre->getIdGenre().'">'.$unGenre->getLibelleGenre().'</a></figcaption></div>';
